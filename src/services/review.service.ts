import { PrismaClient, Review, UserRole, Prisma } from "@prisma/client"; // Import Prisma
...
export const createReview = async (
  ...
  if (!orderItem) {
    throw Object.assign(
      new Error(`Order item with ID ${orderItemId} not found.`),
      { statusCode: 404 }
    );
  }
  if (orderItem.order.userId !== userId) {
    throw Object.assign(
      new Error(`Not authorized to review this order item.`),
      { statusCode: 403 }
    );
  }
  ...
  if (orderItem.review) {
    throw Object.assign(
      new Error(`A review already exists for this order item.`),
      { statusCode: 409 }
    ); // Conflict
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const avgRating = stats._avg.rating
      ? new Decimal(stats._avg.rating).toDecimalPlaces(2)
      : new Decimal(0); // Use Decimal, handle null
...
export const getReviewsByProductId = async (
  productId: string,
  options: any = {}
): Promise<{ data: Review[]; total: number; average: number | null }> => {
  const { skip, take, orderBy } = parsePaginationAndSorting(options);

  const where: Prisma.ReviewWhereInput = { productId };

  const findManyArgs: Prisma.ReviewFindManyArgs = {
    where,
    skip,
    take,
    include: {
      user: { select: { id: true, name: true, imgUrl: true } },
      // productVariant: { select: { id: true, name: true } } // Optional
    },
    orderBy, // Use orderBy from helper
  };

  const [reviews, totalReviews, ratingStats] = await prisma.$transaction([
    prisma.review.findMany(findManyArgs),
  ...
  return {
    ...
    average: ratingStats._avg.rating
      ? new Decimal(ratingStats._avg.rating).toNumber()
      : null,
  };
};
...
export const updateReview = async (
  ...
  if (review.userId !== userId && userRole !== UserRole.ADMIN) {
    throw Object.assign(new Error(`Not authorized to update this review.`), {
      statusCode: 403,
    });
  }
...
export const deleteReview = async (
  ...
  if (review.userId !== userId && userRole !== UserRole.ADMIN) {
    throw Object.assign(new Error(`Not authorized to delete this review.`), {
      statusCode: 403,
    });
  }
